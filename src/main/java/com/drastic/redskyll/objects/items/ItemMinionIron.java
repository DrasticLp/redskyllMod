...
public class ItemMinionIron extends Item implements IHasModel
{
    public ItemMinionIron(String name)
    {
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        ...
        if(!worldIn.isRemote)
        {
            if(hasMinion.get() == 0)
            {               
                EntityMinionIron minion = new EntityMinionIron(worldIn, pos.getX(), pos.getY() + 1, pos.getZ());
