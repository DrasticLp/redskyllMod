...
public class ChunkGeneratorMars implements IChunkGenerator
{
    ...
    public void setBlocksInChunk(int x, int z, ChunkPrimer primer)
    {
        ...
        for(int i = 0; i < 4; ++i)
        {
            ...
            for(int l = 0; l < 4; ++l)
            {
                int i1 = (j + l) * 33, j1 = (j + l + 1) * 33, k1 = (k + l) * 33, l1 = (k + l + 1) * 33;

                for(int i2 = 0; i2 < 32; ++i2)
                {
                    double d0 = .125D, d1 = this.heightMap[i1 + i2], d2 = this.heightMap[j1 + i2], d3 = this.heightMap[k1 + i2], d4 = this.heightMap[l1 + i2], d5 = (this.heightMap[i1 + i2 + 1] - d1) * d0, d6 = (this.heightMap[j1 + i2 + 1] - d2) * d0, d7 = (this.heightMap[k1 + i2 + 1] - d3) * d0, d8 = (this.heightMap[l1 + i2 + 1] - d4) * d0;

                    for(int j2 = 0; j2 < 8; ++j2)
                    {
                        double d9 = .25D, d10 = d1, d11 = d2, d12 = (d3 - d1) * d9, d13 = (d4 - d2) * d9;

                        for(int k2 = 0; k2 < 4; ++k2)
                        {
                            double d14 = .25D, d16 = (d11 - d10) * d14, lvt_45_1 = d10 - d16;
            ...
            for(int l = 0; l < 5; ++l)
            {
                ...
                for(int j1 = -2; j1 <= 2; ++j1)
                {
                    for(int k1 = -2; k1 <= 2; ++k1)
                    {
                        ...
                        float f5 = 0.0F + biome1.getBaseHeight() * 1.0F, f6 = 0.0F + biome1.getHeightVariation() * 1.0F;
                ...
                ++j;
                double d8 = (double)f3, d9 = (double)f2;
                d8 = d8 + d7 * .2D;
                d8 = d8 * 8.5D / 8.0D;
                ...
                for(int l1 = 0; l1 < 33; ++l1)
                {
                    ...
                    double d2 = this.minLimitRegion[i] / 512D, d3 = this.maxLimitRegion[i] / 512D, d4 = (this.mainNoiseRegion[i] / 10D + 1D) / 2D, d5 = MathHelper.clampedLerp(d2, d3, d4) - d1;
    ...
    public void populate(int x, int z)
    {
        ...
        long k = this.rand.nextLong() / 2L * 2L + 1L, l = this.rand.nextLong() / 2L * 2L + 1L;
        this.rand.setSeed((long)x * k + (long)z * l ^ this.world.getSeed());

        int i1 = this.rand.nextInt(16) + 8, j1 = this.rand.nextInt(256), k1 = this.rand.nextInt(16) + 8;
