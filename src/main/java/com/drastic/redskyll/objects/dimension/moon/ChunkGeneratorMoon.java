...
public class ChunkGeneratorMoon implements IChunkGenerator
{
    ...
    public ChunkGeneratorMoon(World world, boolean mapFeaturesEnabled, long seed, BlockPos spawn)
    {
        ...
        spawnableCreatureList.add(new SpawnListEntry(EntityMinionDiamond.class, 100, 1, 2));
    ...
    public Chunk generateChunk(int x, int z)
    {
        ...
        {
            abyte[i] = (byte)Biome.getIdForBiome(BiomeInit.MOON);
        }
    ...
    public void createStructure(int x, int y, int z)
    {
        final WorldServer worldServer = FMLCommonHandler.instance().getMinecraftServerInstance().getWorld(0);
        MinecraftServer mcServer = world.getMinecraftServer();

        TemplateManager manager = worldServer.getStructureTemplateManager();
        ResourceLocation location = new ResourceLocation(Reference.MODID, "autel");

        final PlacementSettings settings = (new PlacementSettings()).setChunk(null).setIgnoreEntities(false).setMirror(Mirror.NONE).setRotation(Rotation.NONE);
        ...
        if(template != null)
        {
            ...
            template.addBlocksToWorldChunk(world, new BlockPos(x, y, z), settings);
        }
    }
    ...
    public void populate(int x, int z)
    {
        ...
        if(x == -5 && z == -4)
        {
            int y = this.calculateGenerationHeight(world, x, z, BlockInit.MOON_ROCK);
            this.createStructure(x, y, z);
        }
