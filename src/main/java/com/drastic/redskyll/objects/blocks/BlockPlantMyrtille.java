...
public class BlockPlantMyrtille extends BlockCrops implements IHasModel
{
    ...
    public BlockPlantMyrtille(String name, Item itemIn, int dropChanceIn)
    {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        ...
        if(this.isMaxAge(state) && !worldIn.isRemote)
        {
            ...
            if(rand.nextInt(this.chance) == 0)
            {
                ...
                {
                    worldIn.spawnEntity(new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(this.drop, 1)));
                }
                return true;
            }
            else
