...
public class BlockPlantKiwi extends BlockCrops implements IHasModel
{
    ...
    public BlockPlantKiwi(String name, Item itemIn, int dropChanceIn)
    {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        ...
        if(this.isMaxAge(state) && !worldIn.isRemote)
        {
            ...
            if(rand.nextInt(this.chance) == 0)
            {
                if(rand.nextInt(5) == 0)
                {
                    worldIn.spawnEntity(new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(ItemInit.KIWI, 3)));
                }
                else if(rand.nextInt(5) == 1 || rand.nextInt(5) == 2)
                {
                    worldIn.spawnEntity(new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(ItemInit.KIWI, 2)));
                }
                else
                {
                    worldIn.spawnEntity(new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(ItemInit.KIWI, 1)));
                }
                return true;
            }
            else
    ...
    public void onBlockDestroyedByPlayer(World worldIn, BlockPos pos, IBlockState state)
    {
        ...
        if(this.isMaxAge(state) && !worldIn.isRemote)
        {
            if(rand.nextInt(this.chance) == 0)
            {
                if(rand.nextInt(5) == 0)
                {
                    worldIn.spawnEntity(new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(this.drop, 4)));
                }
                else if(rand.nextInt(5) == 1 || rand.nextInt(5) == 2)
                {
                    worldIn.spawnEntity(new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(this.drop, 3)));
                }
                else
                {
                    worldIn.spawnEntity(new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(this.drop, 2)));
                }
            }
            else
            {
