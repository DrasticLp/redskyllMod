...
public class CommandEcAdmin extends CommandBase
{
    @Override
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException
    {
        if(sender instanceof EntityPlayer)
        {
            ...
            if(args.length == 0)
            {
                self.sendMessage(new TextComponentString(TextFormatting.RED + "Vous devez utiliser la commande comme ceci : /ecadmin [joueur]"));
            }
            else
            {
                if(args.length == 1)
                {
                    if(Arrays.asList(server.getOnlinePlayerNames()).contains(args[0]))
                    {
                        ...
                        {
                            self.displayGUIChest(other.getInventoryEnderChest());
                        }
                    }
                }
            }
